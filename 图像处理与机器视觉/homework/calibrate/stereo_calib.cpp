...
static int print_help(char** argv)
{
    ...
    cout << "Usage:\n " << argv[0] << " -w=<board_width default=9> -h=<board_height default=6>"
        <<" -t=<pattern type: chessboard or charucoboard default=chessboard> -s=<square_size default=2.7> -ms=<marker size default=0.5>"
...
StereoCalib(const vector<string>& imagelist, Size inputBoardSize, string type, float squareSize, float markerSize, cv::aruco::PredefinedDictionaryType arucoDict, string arucoDictFile, bool displayCorners = false, bool useCalibrated=true, bool showRectified=true)
{
    ...
    for( i = j = 0; i < nimages; i++ )
    {
        for( k = 0; k < 2; k++ )
        {
            // const vector<string> a = imagelist;
            const string& filename = imagelist[i*2+k];
            // cout<<filename;
            Mat img = imread(filename, IMREAD_GRAYSCALE);
            // Mat img = imread("/home/melo/work/stereo/20240628-113135/left_4.jpg", IMREAD_GRAYSCALE);
            // imwrite("/home/melo/work/stereo/img.jpg", img);
            if(img.empty())
                break;
            if( imageSize == Size() )
                imageSize = img.size();
...
// epipolar geometry constraint: m2^t*F*m1=0
    ...
    FileStorage fs("/home/melo/work/stereo/intrinsics.yml", FileStorage::WRITE);
    if( fs.isOpened() )
    {
    ...
    fs.open("/home/melo/work/stereo/extrinsics.yml", FileStorage::WRITE);
    if( fs.isOpened() )
    {
...
int main(int argc, char** argv)
{
    ...
    string imagelistfn;
    bool showRectified;
    cv::CommandLineParser parser(argc, argv, "{w|8|}{h|5|}{t|chessboard|}{s|2.7|}{ms|0.5|}{ad|DICT_4X4_50|}{adf|None|}{nr||}{help||}{@input|/home/melo/work/stereo/new_calib.xml|}");
    if (parser.has("help"))
        return print_help(argv);
    showRectified = !parser.has("nr");
    // imagelistfn = "/home/melo/work/stereo/20240628-113135";
