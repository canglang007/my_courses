...
{
	const int rows = disp_e.rows;
	const int cols = disp_e.cols;
	const int N = rows * cols;
	const int D_tolerance = 1;

	//��������������
	int e_sum = 0;
	for (int i = 0; i < rows; i++) {
		for (int j = 0; j < cols; j++) {
			if ((disp_e.at<uchar>(i, j) - disp_g.at<uchar>(i, j)) > D_tolerance)
				e_sum++;
		}
	}
	return 100.0 * e_sum / N;
}
...
double eRMS(const Mat& disp_e, const Mat& disp_g)
{
	...
	for (int i = 0; i < rows; i++) {
		for (int j = 0; j < cols; j++) {
			int diff = (disp_e.at<uchar>(i, j) - disp_g.at<uchar>(i, j));
			sum += diff * diff;
		}
	}
	...
	if (type == CV_8U)
	{
		...
		for (int i = 0; i < height; i++)
		{
			for (int j = 0; j < width; j++)
			{
				...
				depthData[id] = ushort((float)f * baseline / ((float)dispData[id]));
			}
		}
	}
	else
...
void census_transform_rxr(const uint8* source, uint32* census,const sint32& width,
	const sint32& height,const sint32& size)
{
	if (source == nullptr || census == nullptr || width < 1u || height < 1u) {
		return;
	}

	const int block_r = size / 2;  // ����뾶����3x3�ķ���뾶Ϊ1
	...
	*/
	...
	// �����ؼ���censusֵ
	for (sint32 i = block_r; i < height - block_r; i++) {
		for (sint32 j = block_r; j < width - block_r; j++) {

			// ��������ֵ
			const uint8 gray_center = source[i * width + j];

			// ������СΪblock_r x block_r�Ĵ������������أ���һ�Ƚ�����ֵ����������ֵ�ĵĴ�С������censusֵ
			uint32 census_val = 0u;
			for (sint32 r = -block_r; r <= block_r; r++) {
				for (sint32 c = -block_r; c <= block_r; c++) {
					census_val <<= 1;
...
#ifdef __GNUC__
    // 在GCC/Clang中直接使用 __builtin_popcount
    #define __builtin_popcount __builtin_popcount
#else
    // 在其他编译器中定义 __builtin_popcount 为 __popcnt
    #define __builtin_popcount __popcnt
#endif

uint8 popcount(uint32 u)
{
